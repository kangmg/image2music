...
- 성능
    - 평가 그래프
        ...
        ![Accs_and_F1.png](./보고서_res/Accs_and_F1.png)
...
- 음원에서 5가지 무드(aggressive, dramatic, happy, romantic, sad)를 분류하는 다중분류 모델
    ...
    ![Untitled](./보고서_res/Untitled1.png)
...
- 모델 선정
    - 성능이 높은 3개 모델 선정 ( Gradient Boosting Classifier, Random Forest Classifier, Extra Trees Classifier) → 0.578

### 데이터 수집 및 데이터 베이스 구축

![Untitled](./보고서_res/Untitled2.png)

- 멜론 뮤직에서 월간 TOP 100 목록 수집 ( 1980 ~ 2023 )
    - TOP 노래에 대한 meta-data 획득 후 `qurey=”제목 + 아티스트명”` 로 검색 → 유튜브 링크 및 음원 데이터 확보
        
        ![Untitled](./보고서_res/Untitled3.png)
...
- Multi-Agent 구조
    ...
    ![Untitled](./보고서_res/Untitled4.png)
...
- `Home`
    ...
    ![Untitled](./보고서_res/Untitled5.png)
    
- `PlayList 추천 페이지`
    
    ![Untitled](./보고서_res/Untitled6.png)
    
- `실제 음악 추천 화면`
    
    ![Untitled](./보고서_res/Untitled7.png)
...
- 사진으로부터 무드를 분석하는 것은 만족할 성능이 나왔지만, 실제 음원을 감정에 분류한다는 것이 생각했던 것보다 어려운 문제였고, 문제 정의가 잘 되지 않아서 모델링 성능을 충분히 개선하지 못함.